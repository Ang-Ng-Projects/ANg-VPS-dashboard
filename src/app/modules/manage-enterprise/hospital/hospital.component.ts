...
export class HospitalComponent implements OnInit {
  ...
  initValues = {
    ...
    formDetails: [
      ...
      {
        label: 'Facility Name',
        controlName: 'facility_name',
        type: 'select',
        list: this.wabaList
      },
     
    ],
    header: ['SNo', "Date & Time", "Mobile Number","WABA No", "Channel", 'Facility Name'], // table headers
  }
  ...
  async getFeedbackFilter() {
    ...
    const visitors: any = await this.enterpriseService.getFacilityFilter(params);
    ...
    if (appiyoError == '0') {
      ...
      this.wabaList = processVariables['facilit_name_list'] || [];
    console.log('test', this.wabaList)
  ...
  async getLiveAgentData(searchData?) {
    ...
    const ticketsData: any = await this.enterpriseService.getHospitalList(params);
    ...
    if (appiyoError == '0' && apiErrorCode == "200") {
      ...
      this.customListDatas = {
        ...
        appointment : true,
        keys: ['id', "created_at", "mobile_number","waba_number", "channel", "facility_name"],  // To get the data from key
      }    
  } else {
  ...
  async onDownloadCsv(event) {
    ...
    if(event.fromDate && event.toDate  ){
        params = {
          "processId":"0c8c9b8eb8cc11eda85d0242ac110002",
          "ProcessVariables":{
                ...event,
                test1: this.totalRecords
              },
         
          "workflowId":"0c8c9b8eb8cc11eda85d0242ac110002",
      ...
      console.log('params', params);
          this.enterpriseService.liveAgentCsvDownload(params).subscribe(visitors => {
            ...
            a.target = '_blank';
            a.download = 'Hospital Location.csv';
            document.body.appendChild(a);
            a.click();
            // var blob = new Blob([visitors], {type: 'text/csv'});
            // FileSaver.saveAs(blob, "Hospital Location.csv");
        })
  }
  else{
