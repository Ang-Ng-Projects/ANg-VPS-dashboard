...
export class PatientComponent implements OnInit {
  ...
  initValues = {
    ...
    header: ['SNo', "Date & Time", "Mobile Number","title",'first name','last name', 'email id', 'emirates id', 'date of birth','gender','country code'], // table headers
  }
  ...
  async getLiveAgentData(searchData?) {
    ...
    const ticketsData: any = await this.enterpriseService.getPatientFilter(params);
    ...
    if (appiyoError == '0' ) {
      ...
      this.customListDatas = {
        ...
        appointment : true,
        keys: ['id', "date", "mobile_number","title",'first_name','last_name', 'email_id', 'emirates_id', 'date_of_birth','gender','country_code'],  // To get the data from key
      }    
  } else {
  ...
  async onDownloadCsv(event) {
    ...
    if(event.fromDate && event.toDate){
        params = {
          "processId":"55d627ecdd2411ee8f440242ac110005",
          "ProcessVariables":{
                ...event,
                test1: parseInt(this.totalRecords)
              },
         
          "workflowId":"65ead03f1baf4f36a5bceaa6",
      ...
      console.log('params', params);
          this.enterpriseService.pharmacyAgentCsvDownload(params).subscribe(visitors => {
            ...
            a.target = '_blank';
            a.download = 'Patient Registration Report.csv';
