...
@Component({
  selector: 'app-analytics-mile',
  templateUrl: './analytics-mile.component.html',
  styleUrls: ['./analytics-mile.component.css']
})
export class AnalyticsMileComponent implements OnInit {
  allTickets: any;
  isNoRecord: boolean;
  header: any;
  table: any;
  ...
  async getLiveAgentData() {
    ...
    const ticketsData: any = await this.enterpriseService.getAnaLive(params);

    console.log('Tickets Data patient', ticketsData)
    ...
    if (appiyoError == '0' && apiErrorCode == "1") {
      ...
      this.allTickets = processVariables['milestoneCountList'] || [];
