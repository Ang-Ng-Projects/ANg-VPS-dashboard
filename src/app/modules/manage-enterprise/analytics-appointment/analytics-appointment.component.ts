...
export class AnalyticsAppointmentComponent implements OnInit {
  ...
  async getChart() {
    ...
    try {
      ...
      const response: any = await this.enterpriseService.getAppAnaChart(params);
      console.log('getChartAppointment', response);
      ...
      if (appiyoError == '0') {
        ...
        const graphCounts = this.processVariables.appointmentList;
