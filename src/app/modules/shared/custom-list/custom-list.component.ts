...
export class CustomListComponent implements OnInit, OnChanges {
  ...
  @Input() set data(value: any) {
    console.log('value', value, "counts", this.count, "data", this.data)
    if (!value) {
      return;
    }
    ...
    this.totalAppointmentRemainder = value?.totalAppointmentRemainder;
  ...
  ngOnInit(): void {
    if (this.title == 'Appointment') {
      var down = window.sessionStorage.getItem('appointment download status')
      if (down == 'Downloaded' || down == '' || down == null) {
        this.isLoading = false
        this.button = 'Download Report';
      } else {
        this.isLoading = true
        this.button = 'Downloading';
      }
    } else if (this.title == 'Feedback') {
      var down = window.sessionStorage.getItem('feedback download status')
      if (down == 'Downloaded' || down == '' || down == null) {
        this.isLoading = false
        this.button = 'Download Report';
      } else {
    ...
    } else if (this.title == 'Live Agent') {
      console.log('tested')
      var down = window.sessionStorage.getItem('live download status')
      if (down == 'Downloaded' || down == '' || down == null) {
        this.isLoading = false
        this.button = 'Download Report';
      } else {
    ...
    } else if (this.title == 'Milestone') {

      var down = window.sessionStorage.getItem('mile download status')
      if (down == 'Downloaded' || down == '' || down == null) {
        this.isLoading = false
        this.button = 'Download Report';
      } else {
    ...
    } else if (this.title == 'Appointment Conversion') {
      var down = window.sessionStorage.getItem('conversion download status')
      if (down == 'Downloaded' || down == '' || down == null) {
        this.isLoading = false
        this.button = 'Download Report';
      } else {
    ...
    else if (this.title == 'Appointment Reminder') {
      var down = window.sessionStorage.getItem('conversion download status')
      if (down == 'Downloaded' || down == '' || down == null) {
        this.isLoading = false
        this.button = 'Download Report';
    ...
    this.options = {
      ...
      endDate: this.dateService.getWhichDay(8),
    ...
    this.toasterService.currentData.subscribe(currentData => {
      if (currentData == 'Downloaded') {
        this.button = 'Download Report';
        this.isLoading = false;
      }
  ...
  public selectedDate(value: any, datepicker?: any) {
    ...
    var b = moment(this.searchFromDate);
    var a = moment(this.searchToDate);
    var output = a.diff(b, 'days')
    var dateCount = output + 1;
    if (dateCount <= 62) {
  ...
  onDownload() {
    ...
    this.searchDatas.button = '';
    this.searchDatas.button = this.button;
  ...
  apply() {
    ...
    if (formValue && Array.isArray(formValue.facility_name)) {
      formValue.facility_name = formValue.facility_name?.map(d => `'${d}'`).join(',')
    }
    ...
    if (formValue && Array.isArray(formValue.buttonResponse)) {
      formValue.buttonResponse = formValue.buttonResponse?.map(d => `'${d}'`).join(',')
    }
    // if (formValue && Array.isArray(formValue.buttonResponse)) {
    //   formValue.buttonResponse = formValue.buttonResponse.join(',');
    // }
    ...
    if (this.title === 'Appointment Reminder') {
      this.searchDatas = {
  ...
  pageChangeEvent(event) {
    ...
    if (this.searchDatas && Array.isArray(this.searchDatas.facility_name)) {
      this.searchDatas.facility_name = this.searchDatas.facility_name?.map(d => `'${d}'`).join(',')
    }
    ...
    if (this.searchDatas && Array.isArray(this.searchDatas.buttonResponse)) {
      this.searchDatas.buttonResponse = this.searchDatas.buttonResponse?.map(d => `'${d}'`).join(',')
    }
    // if (this.searchDatas && Array.isArray(this.searchDatas.buttonResponse)) {
    //   this.searchDatas.buttonResponse = this.searchDatas.buttonResponse.join(',');
    // }
